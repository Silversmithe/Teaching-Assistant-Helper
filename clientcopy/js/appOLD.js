...
class AObject {
    // announcements
    constructor(id, timestamp, name, announcement){
        this.id = id;
        this.timestamp = timestamp;
        this.name = name;
    ...
    $.ajax({ url: 'pull_session_info.php',
     ...
     success: function(output) {
        ...
        for(var i=0; i<aarray.length; i++){
            ...
            AArray.push(new AObject(announce.id, announce.timestamp, announce.name, announce.announcement));
        }
        ...
        AArray.forEach(function(item){
            ...
            var $time = $("<i>").text(item.timestamp);
            var $announce = $("<blockquote>").text(item.name + ": " + item.announcement);
            var $div = $("<div>", {id: item.id, class: "announcement well well-sm"}).append($time, $announce);
...
$(document).on('click', '#response-submit', function(){
    ...
    $.ajax({ url: 'push_session_info.php',
         data: {action: 'push_qa', qaID: qID, answer: ans},
         type: 'post',
         success: function(output) {
             alert(output);
             // close the modal
